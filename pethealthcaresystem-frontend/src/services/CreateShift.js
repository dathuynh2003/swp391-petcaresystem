...
const CreateShift = () => {
  ...
  const addShift = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/shifts/add', newShift, { withCredentials: true });
      if (response.data) {
  ...
  const deleteShift = async (shiftId) => {
    try {
      const response = await axios.delete(`http://localhost:8080/shifts/delete/${shiftId}`, { withCredentials: true });
      if (response.data) {
